...
const TransferPage = () => {
    ...
    return (
        <ContainerSession backHomePage={true} titleHeader="Transferência" NameIcon="circle-question">
            <StatusBar barStyle="dark-content" />
            <SafeAreaView>
                <View>
                    <ButtonApp icon="money-bill-transfer" text="Nova transferência" submit={handleNewTransfer} />
                </View>
                <View style={styles.containerInputSearch}>
                    <FontAwesome6 name="magnifying-glass" size={22} color={colors.disableBtn} />
                    <TextInput
                ...
                <ScrollView style={styles.containerListContact}>
                    {
                        filteredContacts.length > 0 ? (
                                ...
                                return (
                                    <View key={index} style={styles.cardContact}>
                                        <View style={styles.initialName}>
                                            <Text style={globalFonts.semiBold16}>{initials.toUpperCase()}</Text>
                                        </View>
                                        <Text style={globalFonts.regular14}>{contato.name}</Text>
                                        <TouchableOpacity>
                                            {contato.favorite ? (
                                                <FontAwesome6 name="star" solid color={colors.primaryBlue} />
                                            ) : (
                            ...
                            <View style={{ width: '100%' }}>
                                <Text style={globalFonts.regular14Gray}>Nenhum contato encontrado.</Text>
                            </View>
                        )
                    }
                </ScrollView>
