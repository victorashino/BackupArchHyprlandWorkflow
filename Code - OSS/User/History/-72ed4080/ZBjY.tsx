...
export const ExtractItem: React.FC<ItemProps> = ({ extract }) => {
  ...
  const navToComprovante = () => {
    if (extract.method == "PIX") {
      if (extract.send == 1) {
        console.error("PIX In: " + (extract.method == "PIX"))
        router.push(`/(home)/extract/comprovante/pixIn?transactionId=${extract.id}`)
      } else {
        console.error("PIX Out: " + (extract.method == "PIX"))
        router.push(`/(home)/extract/comprovante/pixOut?transactionId=${extract.id}`)
      }
      console.log(extract.id)
    } else if (extract.method == "TAX") {
      if (extract.send == 1) {
        router.push(`/(home)/extract/comprovante/tevIn?transactionId=${extract.id}`)
        console.log(extract.id)
      } else {
      ...
      if (extract.send == 1) {
        console.error("TED" + extract.method)
