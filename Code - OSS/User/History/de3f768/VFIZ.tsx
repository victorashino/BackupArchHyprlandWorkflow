...
export const fetchUserInfo = async (
    ...
    try {
      const user = await UserApi.info(); // Chama a API para obter informações do usuário
      setUserInfo(user.data); // Atualiza o estado com as informações do usuário
      setBalance(user.data.amount); // Atualiza o estado com o saldo
    } catch (error) {
      console.error('Erro ao buscar informações do usuário:', error);
    } finally {
      setLoadingBalance(false); // Garante que o loading é desativado, mesmo se ocorrer um erro
    }
}
