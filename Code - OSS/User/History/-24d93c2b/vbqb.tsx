...
const TwoDatePickers: React.FC = () => {
    const { start, end, setStart, setEnd, dropdownDisabled, updateDropdownDisabled } = useExtract();
    ...
    const onStartDateChange = (event: any, selectedDate?: Date) => {
        ...
        if (selectedDate) {
            ...
            setStart(formatDateForAPI(selectedDate)); // Atualiza a data no formato yyyy-mm-dd
            updateDropdownDisabled(true);
        }
    };
    ...
    const onEndDateChange = (event: any, selectedDate?: Date) => {
        ...
        if (selectedDate) {
            ...
            setEnd(formatDateForAPI(selectedDate)); // Atualiza a data no formato yyyy-mm-dd
            updateDropdownDisabled(true);
        }
    };
    ...
    useEffect(() => {
        if (startDate) {
            setStart(formatDateForAPI(startDate));
        }
        if (endDate) {
            setEnd(formatDateForAPI(endDate));
        }
    ...
    return (
        <View style={styles.container}>
            ...
            >
                ...
                <Row justify='center'>
                    <Text style={[fonts.semiBold16, styles.date]}>{formatDateForDisplay(startDate)}</Text>
                    {showStartDatePicker && (
                        <DateTimePicker
            ...
            >
                ...
                <Row justify='center'>
                    <Text style={[fonts.semiBold16, styles.date]}>{formatDateForDisplay(endDate)}</Text>
                    {showEndDatePicker && (
                        <DateTimePicker
