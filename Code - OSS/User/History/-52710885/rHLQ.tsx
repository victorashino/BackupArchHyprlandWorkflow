...
const DepositPage: React.FC = () => {
  ...
  const handlePress = (path: Href<string | object>) => {
    router.push(path); // preserva o histórico de navegação
  };
  ...
  return (
    <ContainerSession backHomePage={true} titleHeader="Depósito" NameIcon="question" clickIcon={() => router.push("/(home)/profile/faq")}>
      ...
      <ButtonApp icon="barcode" text="Gerar boleto de recarga" submit={handleGenerateBoleto} />
      <ButtonApp icon="qrcode" text="Gerar QR Code" submit={handleGenerateQRCode} />
    </ContainerSession>
  );
};
