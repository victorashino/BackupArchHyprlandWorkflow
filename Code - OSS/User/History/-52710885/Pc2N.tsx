...
const DepositPage: React.FC = () => {
  ...
  const handlePressWithoutPixKeyCheck = (path: Href<string | object>) => {
    router.replace(path);
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <TouchableOpacity style={styles.btnBack} onPress={() => handlePressWithoutPixKeyCheck('/(home)/homePage')}>
          <FontAwesome6 name="chevron-left" size={20} color={colors.gray} />
        </TouchableOpacity>
        <Text style={[styles.title, fonts.bold24]}>Depósito</Text>
        <TouchableOpacity onPress={() => handlePressWithoutPixKeyCheck('/(home)/notification')}>
      ...
      {/* Botão que verifica se há chave Pix */}
      <ButtonApp 
        icon="barcode" 
