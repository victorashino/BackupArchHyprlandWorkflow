...
const DepositPage: React.FC = () => {
  ...
  // Função para lidar com o clique do botão "Copiar dados da conta"
  const handleCopy = () => {
    if (data?.pix_key) {
      copyToClipboard(
  ...
  return (
    <View style={styles.container}>
      ...
      <ButtonApp 
        ...
        submit={!data?.pix_key && handleCopy}
      />

      <ButtonApp 
        icon="barcode" 
        text="Gerar boleto de recarga" 
        submit={!!data?.pix_key && handlePress("/(home)/deposit/Boleto")} 
      />

      <ButtonApp 
        icon="qrcode" 
        text="Gerar QR Code" 
        submit={() => handlePress("/(home)/deposit/QRCode")}
      />
    </View>
  );
}
