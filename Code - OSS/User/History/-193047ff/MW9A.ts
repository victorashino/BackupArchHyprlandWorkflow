...
class UserApi {
  ...
  public async info() {
    ...
    // Verifica se a resposta contém o objeto com as informações corretas
    if (res.data && !res.data.message) {
      // Retorna a resposta completa, desde que não contenha "message"
      return res.data;
    } else {
      // Se a resposta contém "message", retorne apenas os dados corretos
      const { message, ...filteredData } = res.data;
      return filteredData;
    }
  }
