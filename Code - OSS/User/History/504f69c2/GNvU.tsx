...
const QRCodeFinal = () => {
    ...
    const exportPDF = async () => {
        ...
        if (!btnDisable) {
            setBtnDisable(true); // Desativa o botão para evitar múltiplos cliques
            ...
            try {
                ...
                const logo = await imageToBase64(logoUri); // Conversão para base64
    
                // Variável para armazenar o QRCode em base64
                let qrCodeBase64 = "";
    
                if (qrCode) {
                    // Limpa o ticket antes de gerar o PDF
                    setRegister((prev: any) => ({
                        ...prev,
                        ticket: "",
                    }));
    
                    // Gera a imagem do QRCode como base64
                    qrCodeRef.current?.toDataURL((data: string) => {
                        qrCodeBase64 = `data:image/png;base64,${data}`;
                        generatePDF(logo, qrCodeBase64); // Chama a função para gerar o PDF
                    });
                }
    
            } catch (error) {
                console.error("Erro ao gerar PDF:", error); // Lida com erros na geração do PDF
            } finally {
                setLoading(false); // Finaliza o estado de carregamento
                setBtnDisable(false); // Reativa o botão
            }
        }
    };
                ...
                <>
                    <View style={styles.header}>
                        <TouchableOpacity
                            ...
                            onPress={() => handleBackPress("/(home)/deposit/QRCode")}
                        >
                    ...
                    {showCopyNotification && (
                        ...
                        >
                            <Notification
                                text={"Código QR copiado com sucesso! :)"}
                                isSuccess={true}
                            />
                        </Animated.View>
                    )}
                    ...
                    <TouchableOpacity>
                        <Text
                            ...
                            onPress={() => [router.replace("/(home)/deposit"), setRegister((prev: any) => ({
                                ...prev,
                                ticket: "",
                            }))]}
                        >
