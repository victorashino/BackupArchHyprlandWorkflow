...
const NotificationPage = () => {
    ...
    const handleNotification = async () => {
        setLoading(true);
        const res = await NotificationApi.listNotification(); // Chamando a API real
        setLoading(false);
        if (res && res.data) {
            setNotificationList(res.data); // Supondo que a resposta tenha os dados em `res.data`
        }
    };

    const deleteNotification = async (id: number) => {
        const res = await NotificationApi.deleteNotification(id); // Chamando a API real para deletar
