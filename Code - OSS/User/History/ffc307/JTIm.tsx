...
export const ExtractProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const fetchExtractData = async () => {
        if (isLoading) return; // Evita chamada se já estiver carregando
        setIsLoading(true);
        try {
            ...
            console.log("fetchExtractData chamado");
    ...
    useEffect(() => {
        fetchUserInfo();
    }, []); // Isso será chamado apenas uma vez na montagem do componente.
    
    useEffect(() => {
        // Verifique se já não está carregando para evitar loops
        if (!isLoading) {
            fetchExtractData();
        }
    }, [startPersonalizado, end, type]); // Chamará `fetchExtractData` somente quando esses valores mudarem.
