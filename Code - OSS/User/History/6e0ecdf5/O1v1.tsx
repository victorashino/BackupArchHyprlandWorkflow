...
const BoletoFinal: React.FC = () => {
  const { copyToClipboard, amount, userInfo, today, tomorrow, handleBackPress, imageToBase64 } = useDeposit();
  ...
  useEffect(() => {
    const fetchBarcode = async () => {
      try {
        ...
        setBarcodeImg(result.barcodeLine);
      } catch (error) {
  ...
  const exportPDF = async () => {
    try {
      ...
      let barCodeBase64 = '';
      if (barcode) {
        barCodeRef.current?.toDataURL((data: string) => {
          barCodeBase64 = `data:image/png;base64,${data}`;
