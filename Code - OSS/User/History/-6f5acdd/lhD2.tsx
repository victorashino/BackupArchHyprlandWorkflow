...
const Password = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    if (password.length < 6) {
      setIsValid(false)
      setErrorMessage(t('Register.Password.mustContainMoreThan6Characters'))
    } else if (!hasLetters || !hasNumbers) {
      setIsValid(false)
      setErrorMessage(t('Register.Password.mustContainLetters'))
      setErrorMessage(t('Register.Password.mustContainNumbers'))
    } else {
      setIsValid(true)
      setErrorMessage('')
    }
  ...
  return (
    <Container className='h-[100vh]'>
      ...
      >
        <Text style={fontStyles.fontBold} className={'text-[#253161]'}>{t('Register.Password.proceed')}</Text>
      </TouchableOpacity>
    </Container>
  )
}
