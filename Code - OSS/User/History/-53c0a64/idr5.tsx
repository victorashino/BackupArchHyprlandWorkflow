...
const ModalFilter: React.FC<ModalFilterProps> = ({
  ...
  const handleStartDateChange = (date: string) => {
    try {
      ...
      if (!isNaN(parsedDate.getTime())) {
        console.log('Data de início selecionada:', parsedDate); // Adiciona este console.log
        setStartDate(date);
        closeStartDatePicker();
        onDateChange(formatarData(parsedDate), endDate); // Atualiza o período selecionado
      } else {
  ...
  const handleEndDateChange = (date: string) => {
    try {
      const parsedDate = new Date(date);
      if (!isNaN(parsedDate.getTime())) {
        console.log('Data de término selecionada:', parsedDate); // Adiciona este console.log
        setEndDate(date);
        closeEndDatePicker();
        onDateChange(startDate, formatarData(parsedDate)); // Atualiza o período selecionado
      } else {
