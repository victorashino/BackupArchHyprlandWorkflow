...
import { format, subDays, isValid, parse } from 'date-fns'; // Importando o método format, subDays, isValid e parse do date-fns
...
interface ModalFilterProps {
  ...
  onDateChange: (startDate: string, endDate: string) => void; // Atualizado para incluir startDate e endDate
}
...
const ModalFilter: React.FC<ModalFilterProps> = ({
  ...
  const handleStartDateChange = (date: string) => {
    try {
      const parsedDate = parse(date, 'yyyy/MM/dd', new Date()); // Tentativa de parse da data no formato especificado
      if (isValid(parsedDate)) {
        setStartDate(parsedDate);
        closeStartDatePicker();
        onDateChange(formatarData(parsedDate), formatarData(endDate)); // Atualiza o período selecionado
      } else {
  ...
  const handleEndDateChange = (date: string) => {
    try {
      ...
      if (isValid(parsedDate)) {
        ...
        closeEndDatePicker();
        onDateChange(formatarData(startDate), formatarData(parsedDate)); // Atualiza o período selecionado
      } else {
