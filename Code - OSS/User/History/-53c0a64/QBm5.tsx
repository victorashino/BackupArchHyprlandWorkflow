...
const ModalFilter: React.FC<ModalFilterProps> = ({
  ...
  // const formatarDate = (data: Date | null): string => {
  //   if (!data) return '';
  ...
  const closeStartDatePicker = () => {setStartDatePickerVisible(false)};
  ...
  const handleStartDateChange = (date: string) => {
    try {
      ...
      if (isValid(parsedDate)) {
        ...
        onDateChange(date, endDate ? formatDate(endDate) : '');
      } else {
  ...
  const handleEndDateChange = (date: string) => {
    try {
      ...
      if (isValid(parsedDate)) {
        ...
        closeEndDatePicker();
        onDateChange(startDate ? formatDate(startDate) : '', date);
      } else {
        ...
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            <Row>
              ...
              >
                <Text>{Translate.Modal.Inputs}</Text>
                <Row marginStart={6}>
                  <Image
                    style={styles.arrow}
              ...
              >
                <Text>{Translate.Modal.Outputs}</Text>
                <Row marginStart={6}>
                  <Image
                    style={styles.arrow}
            ...
            <TouchableOpacity style={styles.btnFilter} onPress={closeModal}>
              <Text style={styles.textBtnFilter}>{Translate.Modal.Filter}</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
