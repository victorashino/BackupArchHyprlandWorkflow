...
interface Extract {
  ...
  const filterExtractsByDateRange = (extracts: Extract[], start: string, end: string, type: string) => {
    const startDate = new Date(start)
    const endDate = new Date(end)

    return extracts.filter(extract => {
        const extractDate = new Date(extract.created)
        return extractDate >= startDate && extractDate <= endDate && extract.method === type
    })
}
