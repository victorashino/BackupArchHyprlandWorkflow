...
const ProfilePage = () => {
  ...
  const copyToClipboard = (text: string, message: string) => {
    ...
    setNotification({ text: message, isSuccess: true });
    Animated.timing(opacity, {
      ...
      setTimeout(() => {
        Animated.timing(opacity, {
          ...
          duration: 500,
          useNativeDriver: true,
        }).start(() => setNotification(null)); // Limpa a notificação após desaparecer
      }, 2000);
    });
  };
  ...
  const activateBiometrics = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        setNotification({ text: "Biometria ativada com sucesso.", isSuccess: true });
      } else {
        setNotification({ text: "Não foi possível ativar a biometria.", isSuccess: false });
      }
    } catch (error) {
      setNotification({ text: "Ocorreu um erro ao tentar ativar a biometria.", isSuccess: false });
